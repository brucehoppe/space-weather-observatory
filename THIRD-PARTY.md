...
Space Weather Observatory is MIT-licensed (see `LICENSE`). It links against or bundles
the third-party components below. This file is generated from `cargo metadata` (Rust
workspace dependency graph) and `npm ls` / `license-checker` (frontend dependencies);
regenerate it before each release with:

```bash
cargo metadata --format-version 1   # Rust dependency licences
npx --yes license-checker --production --json   # npm dependency licences
```

All listed licences are permissive (MIT, Apache-2.0, BSD, ISC, Zlib, MPL-2.0,
Unicode-3.0, CDLA-Permissive-2.0, Unlicense) — none impose copyleft obligations on this
application's own source. Full licence texts are available from each crate's/package's
repository or from `crates.io` / `npmjs.com`.

## Data and imagery sources (not code, but attributed on screen)

| Source | Licence / terms | Where credited |
|---|---|---|
| NOAA SWPC (`services.swpc.noaa.gov`) | U.S. Government work, public domain | Sources page, footer |
| NASA SDO/AIA imagery via Helioviewer | NASA data policy (public); Helioviewer API | Credit line on every image |
| Natural Earth coastlines (via `world-atlas`) | Public domain | Aurora view, Sources page |
...
Build-time only (not shipped in the desktop binary): `@tauri-apps/cli`, `typescript`, `vite`, `@types/topojson-client` — all MIT.

## Desktop backend and core (Rust crates)

540 crates resolved across the Cargo workspace (`crates/swo-core`, `src-tauri`), including
transitive and platform-specific (Windows/Linux/Android/iOS) dependencies pulled in by Tauri.
Not every crate below is compiled into every platform's binary, but every crate that
Cargo can resolve for the workspace is listed for completeness.
